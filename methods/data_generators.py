...
class ISICImageDataset(tf.keras.utils.Sequence):
    def __init__(self, args):
        self.fids, self.labels, self.class_names, self.skip_class = (
            common.load_dataset_ISIC(
                args.dataset_root, args.is_train, args.num_classes, args.skip_class
            )
        )
    ...
    def __getitem__(self, idx):
        ...
        for thisI, j in enumerate(i):
            ...
            if self.augment and self.is_train == 1:
                thisImg = thisImg.crop(
                    (
                        np.shape(thisImg)[1] // 100 * random.randint(1, 10),
                        np.shape(thisImg)[0] // 100 * random.randint(1, 10),
                        np.shape(thisImg)[1]
                        - np.shape(thisImg)[1] // 100 * random.randint(1, 10),
                        np.shape(thisImg)[0]
                        - np.shape(thisImg)[0] // 100 * random.randint(1, 10),
                    )
                )  # Center crop taking out up to 10% of the image dimension
            ...
            Xs[thisI,] = thisImg
            Ys[thisI, :] = np.asarray(
                self.labels[self.fids[j][self.fids[j].rfind("/") + 1 :]]
            )
