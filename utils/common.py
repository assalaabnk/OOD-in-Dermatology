...
def check_directory(arg, access=os.W_OK, access_str="writeable"):
    ...
    raise ArgumentTypeError(
        "The provided string `{}` is not a valid {}" " path.".format(arg, access_str)
    )
...
def number_greater_x(arg, type_, x):
    try:
        ...
        raise ArgumentTypeError(
            'The argument "{}" is not an {}.'.format(arg, type_.__name__)
        )
    ...
    if value > x:
        ...
        raise ArgumentTypeError(
            "Found {} where an {} greater than {} was "
...
def load_dataset_ISIC(root, is_train, num_classes, skip_class, fail_on_missing=True):
    ...
    with open(label_root) as csv_file:
        ...
        labels_all = {
            row["image"]: [int(float(row[thisclass])) for thisclass in class_names]
