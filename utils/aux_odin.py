...
def calculate_metrics(scores_in, scores_out, optimal_delta, num_subsets=4):
    ...
    for i in range(num_subsets):
        ...
        y_true = np.concatenate(
            [np.repeat(1, scores_in_test.size), np.repeat(0, scores_out_test.size)]
        )
        ...
        y_true2 = np.concatenate(
            [np.repeat(0, len(scores_in)), np.repeat(1, len(scores_out))]
        )
    ...
    print(
        "Std Deviation of AUC: {:.4f}, Std Deviation of F1 Score: {:.4f}".format(
            std_auc, std_f1
        )
    )
